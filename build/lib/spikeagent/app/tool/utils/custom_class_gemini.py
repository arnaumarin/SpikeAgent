...
class GoogleGenerativeAIError(Exception):
    """Base exception for Google Generative AI errors."""
    pass

SafetySettingDict = Dict[str, Any]
_ToolDict = Dict[str, Any]
_ToolConfigDict = Dict[str, Any]
_ToolChoiceType = Union[str, bool]

def get_client_info(name: str) -> Any:
    """Simple client info implementation."""
    return None

class _BaseGoogleGenerativeAI(BaseModel):
    """Minimal base class for Google Generative AI."""
    google_api_key: Optional[Union[str, SecretStr]] = Field(default=None, alias="api_key")
...
# Add a module-level function to convert dictionaries to Schema objects
def dict_to_schema(param_dict: Dict[str, Any]) -> 'Schema':
    ...
    if "type" in param_dict:
        type_str = param_dict["type"].upper()
        # Map type strings to Type enum values
        type_mapping = {
            "STRING": Type.STRING,
            "NUMBER": Type.NUMBER,
        ...
        if type_str in type_mapping:
            schema.type_ = type_mapping[type_str]
    
    if "properties" in param_dict:
        for prop_name, prop_schema in param_dict["properties"].items():
            schema.properties[prop_name] = dict_to_schema(prop_schema)
    
    if "required" in param_dict:
        schema.required.extend(param_dict["required"])
    
    # Handle 'items' field for array types
    if "items" in param_dict:
        schema.items = dict_to_schema(param_dict["items"])
    
    # Handle other common schema fields
    if "description" in param_dict:
        schema.description = param_dict["description"]
    
    if "enum" in param_dict:
        schema.enum.extend(param_dict["enum"])
    
    return schema

def schema_to_dict(schema: 'Schema') -> Dict[str, Any]:
    """Convert a Schema object back to a dictionary."""
    if not schema:
        return {}
    ...
    if hasattr(schema, 'type_') and schema.type_:
        type_mapping = {
            Type.STRING: "string",
            Type.NUMBER: "number", 
        ...
        if schema.type_ in type_mapping:
            result["type"] = type_mapping[schema.type_]
    
    # Convert properties
    if hasattr(schema, 'properties') and schema.properties:
        result["properties"] = {}
        for prop_name, prop_schema in schema.properties.items():
            result["properties"][prop_name] = schema_to_dict(prop_schema)
...
def convert_to_genai_function_declarations(tools: Sequence[Union[_ToolDict, GoogleTool]]) -> GoogleTool:
    """Convert tools to GoogleTool with function declarations."""
    if not tools:
        return GoogleTool()
    ...
    for tool in tools:
        if isinstance(tool, GoogleTool):
            # Already a GoogleTool, return as-is
            return tool
        elif isinstance(tool, dict):
            # Convert dict tool to FunctionDeclaration
            if "function" in tool:
                ...
                parameters_schema = dict_to_schema(parameters_dict) if parameters_dict else None
            ...
            # Callable - extract from docstring and annotations
            import inspect
            ...
            for param_name, param in sig.parameters.items():
                ...
                parameters_dict["properties"][param_name] = param_schema
                
                if param.default == inspect.Parameter.empty:
                    parameters_dict["required"].append(param_name)
            
            parameters_schema = dict_to_schema(parameters_dict)
...
def tool_to_dict(tool: GoogleTool) -> Dict[str, Any]:
    """Convert GoogleTool to dictionary representation."""
            ...
            "function_declarations": [
                {
                    ...
                    "parameters": schema_to_dict(fd.parameters) if fd.parameters else {}
                }
...
def _tool_choice_to_tool_config(tool_choice: Union[str, bool], all_names: List[str]) -> Dict[str, Any]:
    """Convert tool_choice to tool_config format."""
    if tool_choice is True or tool_choice == "auto":
        return {
...
def is_basemodel_subclass_safe(cls: Any) -> bool:
    """Safely check if a class is a Pydantic BaseModel subclass."""
    try:
...
class ImageBytesLoader:
    """Minimal image loader implementation."""
    def _bytes_from_url(self, url: str) -> bytes:
        import httpx
        return httpx.get(url).content
    
    def load_part(self, url: str) -> Part:
        if url.startswith("data:"):
            ...
            # Handle regular URLs
            bytes_ = self._bytes_from_url(url)
            mime_type = "image/jpeg"  # Default
        
        return Part(inline_data={"data": bytes_, "mime_type": mime_type})

def image_bytes_to_b64_string(data: bytes, image_format: str) -> str:
    """Convert image bytes to base64 string."""
    b64_string = base64.b64encode(data).decode("utf-8")
    return f"data:image/{image_format};base64,{b64_string}"
        ...
        if isinstance(part, str):
            ...
            if _is_lc_content_block(part):
                if part["type"] == "text":
                    ...
                    media_part = Part()
                    media_part_content = part.get("content", part)
                    if isinstance(media_part_content, dict):
                        media_part.inline_data = Blob(
                            data=base64.b64decode(
                                media_part_content.get("data", media_part_content)
                            ),
                            mime_type=mime_type,
                        )
                    elif "data" in part:
                        # Handle base64 data
                        media_part.inline_data = Blob(
                            data=base64.b64decode(part["data"]), mime_type=mime_type
                        )
                    ...
                    # Handle thinking content by converting to text (thinking is not preserved in parts)
                    thinking_text = part.get("thinking", "")
                    if thinking_text:
                        parts.append(Part(text=f"[Thinking: {thinking_text}]"))
                else:
                    raise ValueError(
                        f"Unrecognized message part type: {part['type']}. Only text, "
                        f"image_url, media, and thinking types are supported."
                    )
            else:
                # Yolo
                logger.warning(
...
class ChatGoogleGenerativeAI_H(_BaseGoogleGenerativeAI, BaseChatModel):
    ...
    def validate_environment(self) -> Self:
        ...
        if google_api_key:
            ...
            metadata.append(("x-goog-api-key", google_api_key))
            self.default_metadata = tuple(metadata)
            
            # Force REST transport when using API keys with custom endpoints
            if client_options.get("api_endpoint") and not self.credentials:
                transport = "rest"
                # Use dummy credentials to prevent default credential lookup
        ...
        # Add credentials (including dummy ones for API key auth)
        if credentials is not None:
            client_kwargs["credentials"] = credentials
        
        self.client = GenerativeServiceClient(**client_kwargs)
    ...
    def async_client(self) -> v1betaGenerativeServiceAsyncClient:
        ...
        if not self.credentials:
            if isinstance(self.google_api_key, SecretStr):
                ...
                google_api_key = self.google_api_key
        # NOTE: async client requires a running event loop
        if not self.async_client_running and _is_event_loop_running():
            # async clients don't support "rest" transport for async operations
            ...
            if google_api_key:
                ...
                # For async client with custom endpoints, we need to handle this differently
                # since async clients have issues with REST transport
                if client_options.get("api_endpoint") and not self.credentials:
                    # Create a dummy credentials object for async client
                    from google.auth.credentials import AnonymousCredentials
                    credentials = AnonymousCredentials()
                else:
                    credentials = self.credentials
            ...
            self.async_client_running = v1betaGenerativeServiceAsyncClient(**client_kwargs)
    ...
    def _prepare_params(
        ...
        gen_config = {
            ...
            for k, v in {
                ...
                "top_p": self.top_p,
                "response_modalities": self.response_modalities,
                # Only include thinking_config if thinking_budget is a positive number
                "thinking_config": (
                    (
                        {"thinking_budget": self.thinking_budget}
                    ...
                    | (
                        {"include_thoughts": self.include_thoughts}
                        if self.include_thoughts is True
                        else {}
                    )
                )
                if (self.thinking_budget is not None and self.thinking_budget > 0) or self.include_thoughts is True
                else None,
            }.items()
            if v is not None
        }
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Convert to Google tool format and then back to OpenAI-style dict format
            google_tool = convert_to_genai_function_declarations(tools)
            google_tool_dict = tool_to_dict(google_tool)
            
            # Convert back to OpenAI-style format that _prepare_request expects
            formatted_tools = []
            if "function_declarations" in google_tool_dict:
                for func_decl in google_tool_dict["function_declarations"]:
                    openai_style_tool = {
                        "type": "function",
                        "function": {
                            "name": func_decl["name"],
                    ...
                    formatted_tools.append(openai_style_tool)
